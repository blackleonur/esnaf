...
const UserCompletedServices = ({navigation}: RegisterScreenProps) => {
  ...
  const handleReject = async () => {
    try {
      const response = await axios.post(
        ...
        {serviceId, statusValue: 5},
      );
      if (response.data.isSuccess) {
        Alert.alert('Başarılı', 'Geçmiş İş Silindi');
  ...
  const handleComment = () => {
    if (serviceDetail) {
      navigation.navigate('UserCommentScreen', {
        ...
        businessId: serviceDetail.businessId,
      });
    }
  };
...
export default UserCompletedServices;
